import {
  Body,
  Controller,
  Get,
  NotFoundException,
  Param,
  Post,
  Request,
  UseGuards,
} from '@nestjs/common';
import {
  ApiBearerAuth,
  ApiOperation,
  ApiResponse,
  ApiTags,
} from '@nestjs/swagger';
import { RolesGuard } from 'src/auth/auth.guard';

import { Dashboard, DashboardUpdate } from './dashboard.dto';
import { DashboardType } from './dashboard.entity';
import { DashboardService } from './dashboard.service';

@ApiTags('Dashboard')
@Controller('dashboard')
export class DashboardController {
  constructor(private readonly dashboardService: DashboardService) {}

  @Get(':type')
  @UseGuards(RolesGuard)
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Get dashboard',
    description: 'Get dashboard by type, for the current user',
  })
  @ApiResponse({
    status: 200,
    description: 'Dashboard object',
    type: Dashboard,
  })
  @ApiResponse({
    status: 404,
    description: 'Dashboard not found',
  })
  async get(
    @Param('type') type: DashboardType,
    @Request() req: { userId: number },
  ): Promise<Dashboard> {
    const result = await this.dashboardService.get(req.userId, type);
    if (!result) {
      throw new NotFoundException('Dashboard not found');
    }
    return new Dashboard(result);
  }

  @Post(':type')
  @UseGuards(RolesGuard)
  @ApiBearerAuth()
  @ApiOperation({
    summary: 'Update dashboard',
    description: 'Update dashboard by type, for the current user',
  })
  @ApiResponse({
    status: 200,
    description: 'Updated dashboard object',
  })
  async update(
    @Param('type') type: DashboardType,
    @Request() req: { userId: number },
    @Body() update: DashboardUpdate,
  ): Promise<void> {
    await this.dashboardService.update(req.userId, type, update);
  }
}
