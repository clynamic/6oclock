...
export const ManifestHealthFrame: React.FC<ManifestHealthFrameProps> = ({
  ...
  const missing = manifest.slices.reduce((acc, s) => acc + s.unavailable, 0);
  ...
  return (
    <PopupState variant="popover" popupId={`popup-${manifest.id}`}>
      {function (popupState) {
        return (
          <Card sx={{ width: '100%' }}>
            <Stack p={2} spacing={1} sx={{ width: '100%' }}>
              ...
              <Stack direction="row" gap={1} sx={{ flexWrap: 'wrap' }}>
                ...
                />
                <Chip icon={<FolderOpen />} label={`${manifest.count} total`} />
                {missing > 0 && (
                  <Chip icon={<ErrorOutline />} label={`${missing} missing`} />
                )}
              </Stack>
              <Box sx={{ height: 4 }} />
              <Box sx={{ width: '100%' }}>
